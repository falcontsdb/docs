...
**分组聚合**时，时间分组还可以搭配填充函数fill一起使用，若某个时间分组的数据为空，由 fill 函数指明该分组的聚合值如何产生。
  - number，&nbsp;如 fill(100), fill(3.14)，表示以 fill 函数指定的数值进行填充
  - linear，&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;如 fill(linear)，表示根据前后分组的聚合结果以线性插值方式进行填充
  - previous，如 fill(previous)，表示用上一个分组的聚合结果进行填充

- 示例
  ```sql
    > select max(water_level) from h2o_feet group by window(2m) fill(previous) limit 4;
    name: h2o_feet
    +------------------+
    | max(water_level) |
    +------------------+
...
- 示例
  ```sql
    > select count(distinct location) from h2o_feet group by window(12m);
    name: h2o_feet
    +--------------------------+
